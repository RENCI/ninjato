...
Module:    myDiscreteFlyingEdges3D.cxx
...
namespace {
	...
	// myDiscreteFlyingEdges3D populates the information in this class and
	// then invokes Contour() to actually initiate execution.
	template <class T>
	class myDiscreteFlyingEdges3DAlgorithm
	{
	public:
		...
		// Setup algorithm
		myDiscreteFlyingEdges3DAlgorithm();
		...
		{
		public:
			myDiscreteFlyingEdges3DAlgorithm<TT> *Algo;
			double Value;
			Pass1(myDiscreteFlyingEdges3DAlgorithm<TT> *algo, double value)
			{
		...
		{
		public:
			Pass2(myDiscreteFlyingEdges3DAlgorithm<TT> *algo)
			{
				this->Algo = algo;
			}
			myDiscreteFlyingEdges3DAlgorithm<TT> *Algo;
		...
		{
		public:
			Pass4(myDiscreteFlyingEdges3DAlgorithm<TT> *algo, double value)
			{
				this->Algo = algo; this->Value = value;
			}
			myDiscreteFlyingEdges3DAlgorithm<TT> *Algo;
		...
		static void Contour(myDiscreteFlyingEdges3D *self, vtkImageData *input,
	...
	template <class T> const unsigned char myDiscreteFlyingEdges3DAlgorithm<T>::
	...
	template <class T> myDiscreteFlyingEdges3DAlgorithm<T>::
		myDiscreteFlyingEdges3DAlgorithm() :XCases(nullptr), EdgeMetaData(nullptr), NewScalars(nullptr),
	...
	template <class T> void myDiscreteFlyingEdges3DAlgorithm<T>::
		...
		ProcessXEdge(double value, T const* const inPtr, vtkIdType row, vtkIdType slice)
	{
		...
		for (vtkIdType i = 0; i < nxcells; ++i, ++ePtr)
		{
			...
			if (s0 != labelValue)
			{
				edgeCase = (s1 != value ? myDiscreteFlyingEdges3DAlgorithm::BothOutside :
					myDiscreteFlyingEdges3DAlgorithm::LeftOutside);
			}
			else // s0 == labelValue
			{
				edgeCase = (s1 != value ? myDiscreteFlyingEdges3DAlgorithm::RightOutside :
					myDiscreteFlyingEdges3DAlgorithm::BothInside);
			}

			this->SetXEdge(ePtr, edgeCase);

			// if edge intersects contour
			if (edgeCase == myDiscreteFlyingEdges3DAlgorithm::LeftOutside ||
				edgeCase == myDiscreteFlyingEdges3DAlgorithm::RightOutside)
			{
	...
	// interfaces the myDiscreteFlyingEdges3D class with the templated algorithm
	// class. It also invokes the three passes of the Flying Edges algorithm.
	template <class T> void myDiscreteFlyingEdges3DAlgorithm<T>::
		Contour(myDiscreteFlyingEdges3D *self, vtkImageData *input, vtkDataArray *inScalars,
			...
			vtkFloatArray *newNormals, vtkFloatArray *newGradients)
	{
		...
		myDiscreteFlyingEdges3DAlgorithm<T> algo;
		...
		for (vidx = 0; vidx < numContours; vidx++)
		{
			...
			printf("EDGEMETADA\n");
			int sum = 0;
			for (int i = 0; i < algo.NumberOfEdges * 6; i++) {
				if (algo.EdgeMetaData[i] != 0) printf("%d: %d\n", i, algo.EdgeMetaData[i]);
				sum += algo.EdgeMetaData[i];
			}
			printf("%d\n", sum);
			printf("##############\n");
			*/
			...
			printf("XCases\n");
			int sum = 0;
			for (int i = 0; i < (algo.Dims[0] - 1)*algo.NumberOfEdges; i++) {
				//if (algo.XCases[i] != 0) printf("%d: %d\n", i, algo.XCases[i]);
				sum += algo.XCases[i];
			}
			printf("%d\n", sum);
			printf("##############3333\n");
...
myDiscreteFlyingEdges3D::myDiscreteFlyingEdges3D()
{
...
vtkMTimeType myDiscreteFlyingEdges3D::GetMTime()
{
...
//----------------------------------------------------------------------------
int myDiscreteFlyingEdges3D::RequestUpdateExtent(
...
//----------------------------------------------------------------------------
int myDiscreteFlyingEdges3D::RequestData(
	...
	vtkInformationVector *outputVector)
{
	...
	{
		vtkTemplateMacro(myDiscreteFlyingEdges3DAlgorithm<VTK_TT>::
...
//----------------------------------------------------------------------------
int myDiscreteFlyingEdges3D::FillInputPortInformation(int, vtkInformation *info)
{
...
//----------------------------------------------------------------------------
void myDiscreteFlyingEdges3D::PrintSelf(ostream& os, vtkIndent indent)
{
